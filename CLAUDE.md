...
- `engine/power.js` — current estimate and the WLED-style limiter behind `/api/power`. Lives in the **sink**, for the same reason brightness does: that is the one place values are post-brightness, clamped, and about to become the bytes Fadecandy receives — so the sibling of "never apply brightness in the compositor" is *never estimate power there either*. Three things it does that WLED's version doesn't, each for a reason that bites if reversed: (1) **the estimate is gamma-aware**, because `fcserver.json` applies `gamma: 2.5` and a whitepoint, so duty is `wp·(v/255)^2.5` and a mid-grey frame draws ~18% of full white, not 50% — summing bytes reads 3–4× high on every ordinary scene and misses why solid white is singular (255 is the one input the curve doesn't attenuate). **The `gamma`/`whitepoint` config must match `fcserver.json`** or every reading is wrong, and nothing will tell you. (2) **The budget is the tighter of a PSU cap and a rail cap** — `(V_oc − V_floor)/R_eff` — because on this panel the binding constraint is IR drop in the shared supply leads, not the supply's rating: ~40mΩ at 14A is half a volt, and the Pi trips undervoltage at ~4.63V well before a 20A PSU runs out. `rail: null` means uncalibrated; `tools/power-sweep.js` fits it on the Pi. (3) **The rescale is `s^(1/gamma)`, not `s`** — a linear factor would over-dim hard (0.66 of the current is a value scale of 0.845, white at byte 215, not 168). Two traps: the frame's total **is not known until its last pixel is written**, so the limiter cannot fold into `setPixel`'s multiply and must be a second pass over the buffer; and truncating rather than rounding that pass is what keeps the result at or under the budget, never over (WLED's `+1` overshoots deliberately, to dodge an integer-truncation-to-zero it needs and we don't). Measurement is unconditional and only the rescale is gated — reading the headroom is the point of having it.
...
- `components/switcher/PowerMeter.jsx` + `lib/power.js` + `components/switcher/PowerConfig.jsx` — the header's power pill and the budget settings. Same shape as the frame-rate pill (pure display logic in `lib`, 1s poll, `document.hidden` care) with one difference: it is **always lit and has no off switch**, because the server measures unconditionally and this is the only place the limiter is visible — the previews are pre-fader by design and never dim. The headline is **predicted volts when the rail is calibrated** and amps otherwise: on a panel bound by supply sag, "4.72 V" is the number that means something. The rail figures are read-only in the UI on purpose — they come from a measurement, and a hand-typed `R_eff` produces a limiter that is confidently wrong.
