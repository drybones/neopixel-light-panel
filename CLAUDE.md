...
- `components/controls/` — schema-driven controls: ColorControl (react-colorful), NumberControl (linear or `atan` perceptual scale — `lib/perceptual.js`, `sliderScalingParam = 6.7975`), RangeControl, EnumSelect, XYPad (live layer render as pad background), AngleDial (draws the wavefronts, not just a knob), NumField, GradientStopsEditor.
