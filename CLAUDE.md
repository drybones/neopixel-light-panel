...
- `engine/scene-store.js` — scene list + active id, `preprocess()` on every write (attaches `_prepared`, `_blend`, `_displayLayers` so the hot loop never parses/filters), debounced node-persist writes (2s trailing; flushed on SIGINT/SIGTERM). Storage key `scenes_v2`; storage dir pinned to `packages/server/.node-persist/`.
