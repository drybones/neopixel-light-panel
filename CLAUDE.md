...
Key modules:
- `app.js` — Express wiring, render-loop tick, scene + settings storage init. "Off" = `activeSceneId null`: one black frame, then the loop fast-exits.
...
- `components/switcher/PowerMeter.jsx` + `lib/power.js` + `components/settings/PowerSettings.jsx` — the header's power pill and the budget settings. Same shape as the frame-rate pill (pure display logic in `lib`, 1s poll, `document.hidden` care) with one difference: it is **always lit and has no off switch**, because the server measures unconditionally and this is the only place the limiter is visible — the previews are pre-fader by design and never dim. The headline is current draw against the PSU-cap budget; there is no rail voltage to lead with — see `engine/power.js` for why that was tried and dropped.
...
- **A new effect needs nothing in the UI** — ParamPanel already walks the schema, and scene cards and the effect picker both render filmstrips.
